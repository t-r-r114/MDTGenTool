...
bool MDTGenTool::generateTerrain(int iterations, double range, int gain, int layers, int edgeSmoothness, bool exportBrush, bool brushMode, bool brushModeTow, const QString &txtPath, const QString &pngPath) {
    ...
    if(exportBrush){
        ...
        for(int y = 0; y < size; y++){
            for(int x = 0; x < size; x++){
                ...
                int brushValue = static_cast<int>(inverted * (1 - weight) + 255 * weight);
        ...
        if(!brushImage.save(brushPath, "PNG")){
            qDebug() << "brush save error";
        }else{
            qDebug() << "brush save succeed:" << brushPath;
        }
    }
